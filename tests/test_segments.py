...
def test_no_controls_yields_one_segment():
    s = _streams(
        time_s=[0, 100, 200],
        distance=[0.0, 1000.0, 2000.0],
        altitude=[10.0, 20.0, 15.0],
    )
    segs = build_segments(s, controls=[])
    assert len(segs) == 1
    seg = segs[0]
...
def test_segments_around_controls():
    ...
    controls = [
        Control(3, 4, 0.0, 0.0, time_s[3], time_s[4]),
        Control(6, 7, 0.0, 0.0, time_s[6], time_s[7]),
    ]
    segs = build_segments(s, controls)
...
def test_missing_optional_streams_yield_none_means():
    ...
    seg = build_segments(s, controls=[])[0]
...
def test_zero_length_segment_skipped():
    ...
    s = _streams(time_s=time_s, distance=distance)
    controls = [Control(0, 1, 0.0, 0.0, 0, 600)]
    segs = build_segments(s, controls)
    assert [seg.label for seg in segs] == ["C1 → End"]
